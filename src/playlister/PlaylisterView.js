    ...
    */
    refreshPlaylist(playlist) {
        ...
        for (let i = 0; i < playlist.songs.length; i++) {
            ...
            anchor.href = "https://www.youtube.com/watch?v=" +song.youTubeId;
            let songTitle = document.createTextNode(song.title + " by " + song.artist);
            anchor.appendChild(songTitle);
    ...
    */
    updateToolbarButtons(model) {
        ...
        if (model.confirmDialogOpen) {
            ...
            this.disableButton("add-song-button");// i think this is right
            this.disableButton("undo-button");
            this.disableButton("redo-button");
            this.disableButton("close-button");
        }
        else if(model.currentlyEditingsongs){
            this.disableButton("add-list-button");
            this.enableButton("add-song-button");
            this.enableButton("undo-button");
            this.enableButton("redo-button");
            this.enableButton("close-button");
            if(tps.mostRecentTransaction == -1){
              this.disableButton("undo-button");  
            }
            if(tps.mostRecentTransaction >= (tps.numTransactions-1)){
                this.disableButton("redo-button")
            }
        }
        else{
    ...
    enableToolbarButtons(model){
        //right now only enables the toolbar
        this.enableButton("undo-button");
        this.enableButton("redo-button");
        this.enableButton("close-button");
    }
