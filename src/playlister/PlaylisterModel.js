...
export default class PlaylisterModel {
    ...
    */
    constructor() {
        ...
        //maybe some songs to help with fool proof design
        this.currentlyEditingsongs = false;
    }
    ...
    getEditSongIndex(){
        //this is an approach to keep track of what song we are editing
        //this way we can have the controller begin to register changes to song
        //another approach could be to have a currentSong
        return this.songEdit;
    }
    ...
    //going to try to add a song here in a function
    addNewSong(songTitle,songArtist,youtubeId){
        if(this.hasCurrentList()){
            let curList = this.currentList.songs;
            let newSong = new Song("song-"+this.currentList.length);
            newSong.setSongArtist(songArtist);
            newSong.setSongTitle(songTitle);
            newSong.setSongYoutubeId(youtubeId);
            //let curListtoEdit = this.currentList;
            curList.push(newSong);
            //this.loadList(this.currentList);
            this.saveLists(this.currentList);
            this.view.refreshPlaylist(this.currentList);
        }
    }
    addNewSongAt(songTitle,songArtist,youtubeId,index){
        if(this.hasCurrentList()){
            let curList = this.currentList.songs;
            let newSong = new Song("song-"+ index);
            newSong.setSongArtist(songArtist);
            newSong.setSongTitle(songTitle);
            newSong.setSongYoutubeId(youtubeId);
            //let curListtoEdit = this.currentList;
            curList.splice(index,0,newSong);
            //this.loadList(this.currentList);
            this.saveLists(this.currentList);
            this.view.refreshPlaylist(this.currentList);
        }
    ...
    deleteSong(songIndex){
        let curSongs = this.currentList.songs;
        
        curSongs.splice(songIndex,1);
        //this.loadList(this.currentList);
        //console.log("The number times this is being called", this.numTimesCalled++);
        this.saveLists(this.currentList);
        this.view.refreshPlaylist(this.currentList);
    }
    ...
    addDeleteSongTransaction(index){
        let transaction = new DeleteSong_Transaction(this,index,this.currentList.getSongAt(index));
        this.tps.addTransaction(transaction);
        this.view.updateToolbarButtons(this);

    }
    addAddSongTransaction(){
        let currentLengthofSongs = this.currentList.songs.length
        let transaction = new AddSong_Transaction(this,currentLengthofSongs);
        this.tps.addTransaction(transaction);
        this.view.updateToolbarButtons(this);
    }
    addEditSong_Transaction(index,oldSong,newSong){
    ...
    // enableToolBar(){
    //     this.view.enableToolbarButtons(this);
    // }
}